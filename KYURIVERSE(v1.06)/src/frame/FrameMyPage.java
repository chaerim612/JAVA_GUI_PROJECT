...
////////////////top 패널 구성///////////////////////////////
		...
		JLabel name = new JLabel("코스 신청 내역");
			...
			public void itemStateChanged(ItemEvent e) { 
				...
				if(revNum.contains(choice.getSelectedItem())) {
						...
						if(index == 0) {
							...
							jCenter.removeAll();
							TextArea myPage = new TextArea("신청 내역을 선택하세요");
						...
						TextArea myPage = new TextArea(
								"◎등  록  번  호 : " + BookDAO.getReserveList().get(index).getBookingNum() + "\n\n"
								+"◎ 개  강  일 자 : " + BookDAO.getReserveList().get(index).getDate() + "\n\n"
								+"◎ 취 업 지 원 처 : " + startPoint+ "\n"
								+"◎ 목 표  기 업 : " + BookDAO.getReserveList().get(index).getGoal() + "\n\n"
								+"◎ 예상 준비 시간 : " + CompanyDAO.searchCompany(BookDAO.getReserveList().get(index).getGoal()).getTime() + "\n\n"
								+"◎ 신  청  좌  석 : " + BookDAO.getReserveList().get(index).getSeatNum() + "\n\n"
